...
template <sids::color::code color> 
...
// typedef
typedef sids::severity_level custom_severity_level;
        ...
        )
{
    ...
    if ( idx < sids::g_LogSeverityLevelString.size() )
    {
        //strm <<" idx = "<<idx <<" ";
        switch (level)
        {
            case custom_severity_level::TRACE :
                strm << write_in<sids::color::FG_BLUE>(sids::g_LogSeverityLevelString.at(idx));
                break;
            ...
            case custom_severity_level::RESULTS :
                strm << write_in<sids::color::FG_MAGENTA>(sids::g_LogSeverityLevelString.at(idx));
                break;

            case custom_severity_level::INFO :
                strm << write_in<sids::color::FG_GREEN>(sids::g_LogSeverityLevelString.at(idx));
                break;    
                
            case custom_severity_level::WARN :
                strm << write_in<sids::color::FG_YELLOW>(sids::g_LogSeverityLevelString.at(idx));
                break;
            ...
            case custom_severity_level::ERROR :
                strm << write_in<sids::color::FG_RED>(sids::g_LogSeverityLevelString.at(idx));
                break;    
                
            case custom_severity_level::NOLOG :
                strm << write_in<sids::color::FG_DEFAULT>(sids::g_LogSeverityLevelString.at(idx));
                break;
    ...
    {
        strm    << write_in<sids::color::FG_RED>("Unknown log level ")
        ...
        )
{
    ...
    std::size_t idx=static_cast< std::size_t >(level);
    if ( idx < sids::g_LogSeverityLevelString.size() )
        strm << sids::g_LogSeverityLevelString.at(idx);
    else
    {
        strm    << write_in<sids::color::FG_RED>("Unknown log level ")
                << "(int level = "<<static_cast< int >(level)
